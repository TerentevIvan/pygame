...
class UI:
    ...
    def quad_select(self, index, options):
        ...
        for col in range(self.cols):
            for row in range(self.rows):
                ...
                i = col + 2 * row
                color = COLORS['gray'] if col == index['col'] and row == index['row'] else (
                    COLORS['black'])
    ...
    def switch(self):
        ...
        if self.switch_index < self.visible_monsters:
            ...
            v_offset = (-(self.switch_index - self.visible_monsters + 1) * rect.height / self.visible_monsters)
    ...
    def update(self):
        self.input()
        self.available_monsters = [monster for monster in self.player_monsters if monster != self.monster and
                                   monster.health > 0]
