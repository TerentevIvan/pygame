...
class Fire(Sprite):
    def __init__(self, surf, pos, groups, player):
        ...
        self.timer = Timer(100, autostert=True, func=self.kill)
        self.y_ofset = pygame.Vector2(0, 8)

        if self.player.flip:
            self.rect.midright = self.player.rect.midleft + self.y_ofset
            self.image = pygame.transform.flip(self.image, True, False)
        else:
            self.rect.midleft = self.player.rect.midright + self.y_ofset
