...
class Game:
    def __init__(self):
        ...
        # timers
        self.bee_timer = Timer(500, func=self.create_bee, autostert=True, repeat=True)
    ...
    def load_assets(self):
        # graphics
        self.player_farames = import_folder('..', 'images', 'player')
    ...
    def setup(self):
        ...
        for obj in tmx_map.get_layer_by_name('Entities'):
            if obj.name == 'Player':
                self.player = Player((obj.x, obj.y), self.all_sprites, self.collision_sprites, self.player_farames,
                                     self.create_bullet)
            if obj.name == 'Worm':
