...
class Ball(pygame.sprite.Sprite):
    def __init__(self, groups, paddle_sprintes, update_score):
        super().__init__(groups)
        self.paddle_sprintes = paddle_sprintes
        ...
        # shadow surf
        self.shadow_surf = self.image.copy()
        pygame.draw.circle(self.shadow_surf, COLORS['ball shadow'], (SIZE['ball'][0] / 2, SIZE['ball'][1] / 2),
                           SIZE['ball'][0] / 2)
    ...
    def collision(self, direction):
        for sprite in self.paddle_sprintes:
            if sprite.rect.colliderect(self.rect):
                if direction == 'horizontal':
                    if self.rect.right >= sprite.rect.left and self.old_rect.right <= sprite.old_rect.left:
                        self.rect.right = sprite.rect.left
                        self.direction.x *= -1
                    if self.rect.left <= self.rect.right and self.old_rect.left >= sprite.old_rect.right:
